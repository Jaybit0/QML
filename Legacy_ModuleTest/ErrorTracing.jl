...
grover_circ = GroverCircuit(2::Int, 4::Int, Vector{GroverBlock}())

hadamard(grover_circ, 3:6)
rotation(grover_circ, 1, control_lanes = 3:4; max_rotation_rad = pi)
rotation(grover_circ, 1, control_lanes = 5:6; max_rotation_rad = pi)
...
out = auto_compute(grover_circ, 1:2, criterion)

#circ = chain(6, put(1:6 => compile_circuit(grover_circ; inv = false)), put(1:6 => compile_circuit(grover_circ; inv = true)))
#out = zero_state(6) |> circ
#println(state(out)[1])
#vizcircuit(chain(6, put(1:6 => compile_circuit(grover_circ; inv = false)), put(1:6 => compile_circuit(grover_circ; inv = true))))
