...
function empty_circuit(model_lanes::Union{AbstractRange, Vector, Int}, param_lanes::Union{AbstractRange, Vector, Int})::GroverCircuit
    ...
    for lane in model_lanes
        if lane in param_lanes
            throw(DomainError(lane, "The lane $lane is both a model and a parameter lane"))
        end
        if lane < 1 || lane > length(model_lanes) + length(param_lanes)
            throw(DomainError(lane, "The lane $lane is out of bounds (must be within lanes 1:" * string(length(model_lanes) + length(param_lanes)) * ")"))
        end
    end
